...
def _must_match_rekey_ack_header(epoch: int, seq: int, *, expected_epoch: int, expected_seq: int) -> None:
    if int(epoch) != int(expected_epoch) or int(seq) != int(expected_seq):
        print(
            "[server167] BAD REKEY ACK HEADER: "
            f"expected_epoch={expected_epoch} got_epoch={epoch} expected_seq={expected_seq} got_seq={seq}"
        )
        raise RuntimeError("BAD REKEY ACK HEADER")
...
async def do_one_rekey_as_server(io: AsyncFrameIO, *, session_id: int, epoch: int, cfg: ProtocolConfig) -> tuple[int, bytes]:
    ...
    _must_match_rekey_ack_header(f.epoch, f.seq, expected_epoch=epoch, expected_seq=1)

    msg = decode_rekey_plaintext(f.payload)

    if isinstance(msg, RekeyCommit):
        print("[server167] PHASE CONFUSION DETECTED: expected=RekeyAck got=RekeyCommit")
        raise RuntimeError("PHASE CONFUSION")
    if not isinstance(msg, RekeyAck):
        print(f"[server167] PHASE CONFUSION DETECTED: expected=RekeyAck got={type(msg).__name__}")
        raise RuntimeError("PHASE CONFUSION")
