...
def _must_match_ack_header(*, f_epoch: int, f_seq: int, expected_epoch: int, expected_seq: int) -> None:
    if int(f_epoch) != int(expected_epoch) or int(f_seq) != int(expected_seq):
        print(
            "[server167] BAD REKEY ACK HEADER: "
            f"expected_epoch={expected_epoch} got_epoch={f_epoch} "
            f"expected_seq={expected_seq} got_seq={f_seq}"
        )
        raise RuntimeError("BAD_REKEY_ACK_HEADER")
...
async def do_one_rekey_as_server(io: AsyncFrameIO, *, session_id: int, epoch: int, cfg: ProtocolConfig) -> tuple[int, bytes]:
    ...
    # Attack-05 core check: ACK header must be bound to current epoch and expected seq
    _must_match_ack_header(f_epoch=f.epoch, f_seq=f.seq, expected_epoch=epoch, expected_seq=1)
